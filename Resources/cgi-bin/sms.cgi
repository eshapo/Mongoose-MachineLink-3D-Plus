...
#sms_log "query str = $QUERY_STRING"
#sms_log "contents len = $CONTENT_LENGTH"
#sms_log "contents type = $CONTENT_TYPE"
#sms_log "cgi cmd = $CMD"
...
read_smsinbox()
{
	...
	for FILENAME in $FILELIST; do
		...
		TEMPSTR=`awk '/From:/ {print $2}' $FILENAME`
		echo "MobNum[$IDX]=\"$TEMPSTR\";"

		TEMPSTR=`awk '/From_SMSC:/ {print $2}' $FILENAME`
		echo "SMSCNum[$IDX]=\"$TEMPSTR\";"

		TEMPSTR=`awk '/Sent:/ {print $2,$3,$4,$5,$7}' $FILENAME`
		echo "TxTime[$IDX]=\"$TEMPSTR\";"

		TEMPSTR=`awk '/Received:/ {print $2,$3,$4,$5,$7}' $FILENAME`
		echo "RxTime[$IDX]=\"$TEMPSTR\";"

		awk '{if (NR>7) print $0}' $FILENAME > /tmp/smstmp && grep . /tmp/smstmp | awk '{printf "%s\\n",$0}!//{print}' > /tmp/smstmp2
		cat /tmp/smstmp2 | sed -r 's/\\n*$//g' > /tmp/smstmp
		cat /tmp/smstmp | sed -e 's/\\/\\\\/g' -e "s/\`/\\\`/g" -e 's/\"/\\\"/g' -e 's/\!/\\\!/g'> /tmp/smstmp2
		MSGBODY=`cat /tmp/smstmp2`
...
if [ "$CMD" = "DELETE_MSG" ]; then
	...
	for tgt in ${fnlist}; do
		...
		rm $tgt
		sms_log "delete message file '$tgt'"
	done
	if [ "$DEL_LIST" != "" ]; then
...
if [ "$CMD" = "DIAG_CONF_SET" ]; then
	...
	while [ "$i" -le "$MAX_WL_TX_IDX" ]; do
		...
		eval newdiagpassword=\${"DiagPassword"$i}
